...
void Reassembler::insert( uint64_t first_index, string data, bool is_last_substring )
{
  ...
  uint64_t max_acceptable_index=writeable_output.bytes_pushed()+writeable_output.available_capacity();//最大可接受索引[这个索引位置是最大的未output_范围的索引]
  ...
  while(it != stroed_segments_.end() && it->first < first_index + data.size()){
    ...
    if(old_first_index+old_data.size()>first_index){
      //如果新片段被现有片段包含，直接返回
      if(old_first_index<=first_index&&old_first_index+old_data.size()>=first_index+data.size()) { return; }
